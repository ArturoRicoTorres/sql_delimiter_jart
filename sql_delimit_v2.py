...
def _retract_past_trailing_comments(text: str, offset: int) -> int:
    """Move an insertion offset left past trailing whitespace and comments.

    sqlglot attaches a comment that follows a statement to that statement, so a
    boundary computed at the statement's end can land *after* a trailing block
    or line comment (e.g. right after ``*/``). Walk back so the ';' is placed
    immediately after the last real code character instead.
    """
    i = offset
    while i > 0:
        j = i
        while j > 0 and text[j - 1] in " \t\r\n":
            j -= 1
        if j > 0 and text[j - 1] == "/" and text.rfind("/*", 0, j) > text.rfind("*/", 0, j - 1):
            # j sits just after a block comment close; jump to its start
        ...
        m = _LINE_COMMENT_RE.search(text, line_start, j)
        if m and m.start() >= line_start:
            # a line comment occupies the tail of this line; jump before it
            i = m.start()
            continue
        return j
    return i
...
def apply_insertions(sql: str, insertions: list[Insertion], annotate: bool) -> str:
    ...
    for ins in sorted(insertions, key=lambda x: x.offset, reverse=True):
        pos = _retract_past_trailing_comments(sql, ins.offset)
